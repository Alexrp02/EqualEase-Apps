...
async function createKitchenRequest(req, res) {
    ...
    try {
        ...
        // Verificar que existan los id de los kitchen orders
        // Recorrer el array de items y comprobar si existen todos ellos
        if (data.orders.length > 0) {
            const errors = [];
            await Promise.all(data.orders.map(async (id) => {
                const kitchenOrderRef = doc(db, "kitchen_orders", id);
                const kitchenOrderSnapshot = await getDoc(kitchenOrderRef);

                if (!kitchenOrderSnapshot.exists()) {
                    errors.push(`Kitchen Order with ID=${id} does not exist.`);
                }
            }));
...
// Get all requests
async function getKitchenRequests(req, res) {
    try {
...
async function getKitchenRequestById(req, res) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            const object = new Kitchen(data);
            res.status(200).json({id: ref.id, ...object });
        } else {
            res.status(404).json({ error: `Kitchen with id=${id} does not exist.`});
        }
    } catch (error) {
...
async function getKitchenRequestsFromStudent(req, res) {
    const id = req.params.id;
    ...
    try {
        ...
        if(snapshot.empty) {
            res.status(400).json({ error: `${collectionName}: student with id ${id} either does not exist or does not have kitchen requests assigned` });
            return;
        }
        ...
        console.error("Error getting kitchen requests from Firestore:", error);
        res.status(500).send("Server error.");  
    }
}

// update kitchen order (similar to create)
async function updateKitchenRequest(req, res) {
    const id = req.params.id;
...
async function deleteKitchenRequest(req, res) {
    const id = req.params.id;
...
// Para ello realiza una consulta en la colección kitchen y si encuentra algún
